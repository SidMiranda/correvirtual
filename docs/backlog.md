...
- [ ] **Painel administrativo para o organizador** — em construção desde 2026-08-29, fatiado. Spec: `docs/specs/painel-admin.md`. Decisão de construir aqui (e não no Cubo): ADR 0006.
  ...
  - [ ] **Dev e produção compartilham o mesmo bucket R2.** Uma imagem enviada testando no local grava no mesmo lugar de onde produção lê. Os ids de evento são diferentes nos dois bancos, então uma imagem de teste do evento 8 do dev apareceria num futuro evento 8 de produção. Não é perda de dado, é imagem errada — mas vale separar (bucket ou prefixo por ambiente) antes de o painel ser usado pra valer.
