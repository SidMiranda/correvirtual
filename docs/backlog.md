...
**BUG-008 — Credencial de produção do Mercado Pago (conta do Uéslei) rejeitada: "Unauthorized use of live credentials"** — achado ao vivo em 2026-08-03 testando o Pix de teste (R$0,05). A API do Mercado Pago recusa com HTTP 401, `code 7`, mensagem `"Unauthorized use of live credentials"`. Não é bug de código — é a conta/aplicação do Mercado Pago do Uéslei que provavelmente não está habilitada pra processar pagamentos "live" (produção) ainda, precisa verificar no painel de desenvolvedor do Mercado Pago dele (ativação da aplicação, verificação de conta/negócio, etc.). Enquanto isso, o usuário só vê a mensagem amigável de instabilidade (ver DEBT-005) — o detalhe técnico fica só no log do servidor.
